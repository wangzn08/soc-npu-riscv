// conv1 (model.1, 16->32 3x3 s2 pad1) @320 via the reusable descriptor runtime
// yolo_run_conv2d_tiled_desc(). Second real layer through the descriptor queue;
// out_c=32 exercises the multi-OC-group strip drain. Per-element golden check.
...
void usercode7(void)
{
    ...
    print_str("YOLO CONV1 @320 DESCRIPTOR-TILED SMOKE (160x160x16 -> 80x80x32)\n");
    ...
    *(volatile uint32_t *)NPU_PERF_CLR = 1u;
    ...
    if (errors == 0u) { print_str("YOLO CONV1 DESC-TILED SMOKE PASS\n"); return; }
    print_str("YOLO CONV1 DESC-TILED SMOKE FAIL errors="); print_dec(errors); print_str("\n");
