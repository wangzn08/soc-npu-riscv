...
void usercode7(void)
{
    ...
    // ---------- validate c2f_6 output vs golden (integration; 6-block drift propagates) ----------
    for (pos = 0u; pos < YOLO_C2F6_SPATIAL && errors <= 16u; pos++)
        for (oc = 0u; oc < YOLO_C2F6_CV2_OC; oc++) {
            int32_t got = rs8(C2F6_OUT, (oc>>4)*YOLO_C2F6_SPATIAL + pos, oc&15u);
            int32_t exp = s8(yolo_c2f6_golden[pos][oc]);
            uint32_t d = ad(got, exp);
            if (d > maxd) maxd = d;
            if (d > 110u) {   /* integration: 6-block preact-scale drift accumulates */
                errors++;
                if (errors <= 8u) {
    ...
    print_str("backbone c2f_6 vs golden maxdiff="); print_dec(maxd); print_str("\n");
    if (errors == 0u) { print_str("YOLO BACKBONE PASS (conv0->...->c2f_6 chained)\n"); return; }
