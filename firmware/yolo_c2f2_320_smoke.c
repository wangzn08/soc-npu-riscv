// c2f_2 (model.2) at full 320 resolution (80x80) via the generic tiled C2f
// runner with EXACT per-layer SiLU LUTs (CTRL[22]). Block input = the real
// conv1 dump (dump320/conv1.bin, baked). Convs run tiled DDR->DDR; CPU does the
// residual add + concat requant. Output compared to the generator's exact-model
// golden (yolo_c2f2_expected_rtl), which aligns to the C float dump within ~13.
...
void usercode7(void)
{
    ...
    print_str("YOLO C2F2 @320 (model.2, exact SiLU) CPU SMOKE\n");
    for (i = 0u; i < YOLO_C2F2_BLKIN_WORDS; i++)
        wrw(IN_DDR, i, yolo_c2f2_blkin_words[i]);
    ...
    cfg.in_ddr=IN_DDR; cfg.cv1_ic=YOLO_C2F2_CV1_IC; cfg.cv1_out_ddr=CV1_OUT;
    ...
    if (!yolo_run_c2f_block(&cfg)) { print_str("  c2f2 runner failed\n"); errors++; }

    for (pos = 0u; pos < YOLO_C2F2_SPATIAL && errors <= 16u; pos++)
        for (oc = 0u; oc < YOLO_C2F2_CV2_OC; oc++) {
            int32_t got = rs8(OUT_DDR, (oc>>4)*YOLO_C2F2_SPATIAL + pos, oc&15u);
            int32_t exp = s8(yolo_c2f2_expected_rtl[pos][oc]);
            if (ad(got, exp) > YOLO_C2F2_RTL_TOL) {
                errors++;
    ...
    if (errors == 0u) { print_str("YOLO C2F2 @320 CPU SMOKE PASS\n"); return; }
    print_str("YOLO C2F2 @320 CPU SMOKE FAIL errors="); print_dec(errors); print_str("\n");
