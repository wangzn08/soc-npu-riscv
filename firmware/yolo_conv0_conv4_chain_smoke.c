// CPU/NPU smoke for conv0 -> conv1 -> conv2 -> slice(s1) -> conv3 -> conv4.
...
static int run_conv0_to_conv3(void)
{
    ...
    return yolo_slice_ddr_to_act(C2_OUT_DDR,
                                 ACT_BASE,
                                 YOLO_CONV3_CHAIN_IN_SPATIAL,
                                 YOLO_CONV3_CHAIN_SRC_GROUP,
                                 1u) &&
           yolo_dma_ddr_to_wgt(C3_WGT_DDR, WGT_BASE, YOLO_CONV3_CHAIN_WGT_WORDS) &&
           yolo_run_conv2d_qparams(ACT_BASE,
                                   WGT_BASE,
                                   OUT_BASE,
                                   YOLO_CONV3_CHAIN_IN_W,
                                   YOLO_CONV3_CHAIN_IN_H,
                                   YOLO_CONV3_CHAIN_IC,
                                   YOLO_CONV3_CHAIN_OC,
                                   YOLO_CONV3_CHAIN_KH,
                                   YOLO_CONV3_CHAIN_KW,
                                   YOLO_CONV3_CHAIN_STRIDE,
                                   YOLO_CONV3_CHAIN_PAD,
...
void usercode7(void)
{
    ...
    print_str("YOLO CONV0->CONV4 C2F-BOTTLENECK CPU SMOKE\n");

    for (i = 0u; i < YOLO_CONV4_CHAIN_WGT_WORDS; i++)
        write_ddr_word128(C4_WGT_DDR, i, yolo_conv4_chain_wgt_words[i]);
    for (i = 0u; i < YOLO_CONV4_CHAIN_OUT_WORDS; i++) {
        const uint32_t zero[4] = {0u, 0u, 0u, 0u};
        write_ddr_word128(C4_OUT_DDR, i, zero);
    }

    if (!run_conv0_to_conv3()) {
        print_str("  prefix conv0->conv3 run failed\n");
        errors++;
    }

    if (errors == 0u) {
        yolo_set_pad_value(YOLO_CONV4_CHAIN_PAD_VALUE);
        yolo_set_silu_requant(YOLO_CONV4_CHAIN_REQUANT_MUL,
                              YOLO_CONV4_CHAIN_REQUANT_SHIFT,
                              YOLO_CONV4_CHAIN_REQUANT_ZP);
        if (!yolo_dma_ddr_to_act(C3_OUT_DDR, ACT_BASE, YOLO_CONV4_CHAIN_ACT_WORDS) ||
            ...
            !yolo_run_conv2d_qparams(ACT_BASE,
                                     WGT_BASE,
                                     OUT_BASE,
                                     YOLO_CONV4_CHAIN_IN_W,
                                     YOLO_CONV4_CHAIN_IN_H,
                                     YOLO_CONV4_CHAIN_IC,
                                     YOLO_CONV4_CHAIN_OC,
                                     YOLO_CONV4_CHAIN_KH,
                                     YOLO_CONV4_CHAIN_KW,
                                     YOLO_CONV4_CHAIN_STRIDE,
                                     YOLO_CONV4_CHAIN_PAD,
                                     yolo_conv4_chain_bias_q,
                                     yolo_conv4_chain_scale_mul,
                                     yolo_conv4_chain_scale_shift,
                                     NPU_CTRL_SILU_EN |
                                     NPU_CTRL_SILU_REQUANT_EN) ||
            !yolo_dma_out_to_ddr(C4_OUT_DDR, OUT_BASE, YOLO_CONV4_CHAIN_OUT_WORDS, 0u)) {
            print_str("  conv4 chain run failed\n");
            errors++;
        }
    }

    for (pos = 0u; pos < YOLO_CONV4_CHAIN_OUT_SPATIAL && errors <= 16u; pos++) {
        for (oc = 0u; oc < YOLO_CONV4_CHAIN_OC; oc++) {
            int32_t got = read_ddr_s8(C4_OUT_DDR, pos, oc);
            int32_t expect = s8(yolo_conv4_chain_expected_rtl[pos][oc]);
            uint32_t diff = abs_diff(got, expect);
            if (diff > YOLO_CONV4_CHAIN_RTL_TOL) {
                errors++;
    ...
    if (errors == 0u) {
        print_str("YOLO CONV0->CONV4 C2F-BOTTLENECK CPU SMOKE PASS\n");
        return;
    }

    print_str("YOLO CONV0->CONV4 C2F-BOTTLENECK CPU SMOKE FAIL errors=");
    print_dec(errors);
    print_str("\n");
