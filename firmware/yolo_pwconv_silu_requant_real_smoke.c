// CPU/NPU smoke for real conv2 pointwise with zero-point correction,
// per-channel qparams, SiLU LUT, and final output-scale/zero-point requant.
...
void usercode7(void)
{
    ...
    print_str("YOLO REAL PWCONV SILU REQUANT CPU SMOKE\n");
    ...
    if (!yolo_dma_ddr_to_act(ACT_DDR, ACT_BASE, YOLO_SILU_REAL_ACT_WORDS) ||
        ...
        print_str("  real silu requant pwconv path failed\n");
        errors++;
    }
    ...
    for (pos = 0u; pos < YOLO_SILU_REAL_OUT_WORDS; pos++) {
        for (oc = 0u; oc < YOLO_SILU_REAL_OC; oc++) {
            ...
            uint32_t expect = yolo_silu_real_expected_requant[pos][oc];
            if (got != expect) {
                errors++;
    ...
    if (errors == 0u) {
        print_str("YOLO REAL PWCONV SILU REQUANT CPU SMOKE PASS\n");
        return;
    }

    print_str("YOLO REAL PWCONV SILU REQUANT CPU SMOKE FAIL errors=");
    print_dec(errors);
    print_str("\n");
