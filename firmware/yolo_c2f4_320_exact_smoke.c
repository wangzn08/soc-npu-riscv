// c2f_4 (model.4, n=2) @320 standalone with exact-SiLU + baked weights (no blob,
// no chain). Input = dump conv6 (baked). Isolates the c2f_4 exact path on RTL.
...
void usercode7(void)
{
    ...
    print_str("YOLO C2F4 @320 EXACT standalone (n=2, baked weights)\n");
    for (i = 0u; i < YOLO_C2F4_BLKIN_WORDS; i++) wrw(IN_DDR, i, yolo_c2f4_blkin[i]);
    ...
    for (pos = 0u; pos < YOLO_C2F4_SPATIAL && errors <= 16u; pos++)
        for (oc = 0u; oc < YOLO_C2F4_CV2_OC; oc++) {
            ...
            if (d > YOLO_C2F4_RTL_TOL) { errors++; if (errors<=4u){ print_str("  pos="); print_dec(pos); print_str(" oc="); print_dec(oc); print_str(" got="); print_dec((uint32_t)got); print_str(" exp="); print_dec((uint32_t)exp); print_str("\n"); } }
        }
    print_str("c2f4 standalone maxdiff="); print_dec(maxd); print_str("\n");
    if (errors == 0u) { print_str("YOLO C2F4 @320 STANDALONE PASS\n"); return; }
    print_str("YOLO C2F4 @320 STANDALONE FAIL errors="); print_dec(errors); print_str("\n");
