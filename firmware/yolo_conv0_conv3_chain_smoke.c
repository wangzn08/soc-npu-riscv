// CPU/NPU smoke for a real conv0 -> conv1 -> conv2 -> C2f-slice -> conv3 chain.
...
void usercode7(void)
{
    ...
    print_str("YOLO CONV0->CONV3 C2F-SLICE CPU SMOKE\n");
    ...
    for (i = 0u; i < YOLO_CONV3_CHAIN_OUT_WORDS; i++) {
        const uint32_t zero[4] = {0u, 0u, 0u, 0u};
        write_ddr_word128(C3_OUT_DDR, i, zero);
    }
    ...
    if (errors == 0u) {
        ...
        if (!yolo_slice_ddr_to_act(C2_OUT_DDR,
                                   ACT_BASE,
            ...
            print_str("  conv3 chain run failed\n");
            errors++;
        }
    }

    for (pos = 0u; pos < YOLO_CONV3_CHAIN_OUT_SPATIAL && errors <= 16u; pos++) {
        for (oc = 0u; oc < YOLO_CONV3_CHAIN_OC; oc++) {
            int32_t got = read_ddr_s8(C3_OUT_DDR, pos, oc);
            int32_t expect = s8(yolo_conv3_chain_expected_rtl[pos][oc]);
            uint32_t diff = abs_diff(got, expect);
            if (diff > YOLO_CONV3_CHAIN_RTL_TOL) {
                errors++;
    ...
    if (errors == 0u) {
        print_str("YOLO CONV0->CONV3 C2F-SLICE CPU SMOKE PASS\n");
        return;
    }

    print_str("YOLO CONV0->CONV3 C2F-SLICE CPU SMOKE FAIL errors=");
    print_dec(errors);
    print_str("\n");
