// c2f_8 (model.8, n=1, 256ch) via the generic C2f runner. half_c=128 so the
// bottleneck m_cv1/m_cv2 (128->128) and cv1/cv2 (256/384) all use OC-chunking.
// Input = conv20 output (baked).
...
#define CV1_OUT    0x400C0000u
#define BN_OUT     0x400E0000u
#define MCV2_DDR   0x40100000u
#define ADD0_DDR   0x40120000u
#define CONCAT_DDR 0x40140000u
...
void usercode7(void)
{
    ...
    print_str("YOLO C2F8 (model.8, n=1, 256ch) CPU SMOKE\n");
    for (i = 0u; i < YOLO_C2F8_BLKIN_WORDS; i++) wrw(IN_DDR, i, yolo_c2f8_blkin_words[i]);
    ...
    for (pos = 0u; pos < YOLO_C2F8_SPATIAL && errors <= 16u; pos++)
        for (oc = 0u; oc < YOLO_C2F8_CV2_OC; oc++) {
            ...
            int32_t exp = s8(yolo_c2f8_expected_rtl[pos][oc]);
            if (ad(got, exp) > YOLO_C2F8_RTL_TOL) {
                errors++;
    ...
    if (errors == 0u) { print_str("YOLO C2F8 CPU SMOKE PASS\n"); return; }
    print_str("YOLO C2F8 CPU SMOKE FAIL errors="); print_dec(errors); print_str("\n");
