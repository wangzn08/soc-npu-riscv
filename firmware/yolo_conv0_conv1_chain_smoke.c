// CPU/NPU smoke for a real conv0 -> conv1 strip chain.
...
void usercode7(void)
{
    ...
    print_str("YOLO CONV0->CONV1 CHAIN CPU SMOKE\n");
    ...
    for (pos = 0u; pos < YOLO_CONV1_CHAIN_OUT_SPATIAL && errors <= 16u; pos++) {
        for (oc = 0u; oc < YOLO_CONV1_CHAIN_OC; oc++) {
            uint32_t group = oc >> 4;
            uint32_t lane = oc & 15u;
            uint32_t word_idx = group * YOLO_CONV1_CHAIN_OUT_SPATIAL + pos;
            int32_t got = read_ddr_s8(C1_OUT_DDR, word_idx, lane);
            int32_t expect = s8(yolo_conv1_chain_expected_rtl[pos][oc]);
            uint32_t diff = abs_diff(got, expect);
            if (diff > YOLO_CONV1_CHAIN_RTL_TOL) {
                errors++;
    ...
    if (errors == 0u) {
        print_str("YOLO CONV0->CONV1 CHAIN CPU SMOKE PASS\n");
        return;
    }

    print_str("YOLO CONV0->CONV1 CHAIN CPU SMOKE FAIL errors=");
    print_dec(errors);
    print_str("\n");
