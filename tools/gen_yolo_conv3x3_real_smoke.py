#!/usr/bin/env python3
"""Generate a real YOLO conv3 3x3 C-reference smoke fixture."""
...
OUT_PATH = ROOT / "firmware" / "yolo_conv3x3_real_data.h"
...
def conv_cref(act: np.ndarray,
    ...
    out = np.zeros((OUT_H, OUT_W, OC), dtype=np.uint8)
    wsum = weights.astype(np.int32).sum(axis=(1, 2, 3))
    for oy in range(OUT_H):
        for ox in range(OUT_W):
            for oc in range(OC):
                ...
                out[oy, ox, oc] = clamp_s8(c_lround(silu / OUT_SCALE + OUT_ZP))
    return out.reshape(SPATIAL, OC)
    ...
    for oy in range(OUT_H):
        for ox in range(OUT_W):
            for oc in range(OC):
                ...
                out[oy, ox, oc] = np.int8(s8(clamp_s8(rq)))
    return out
...
def main() -> None:
    ...
    rtl_s = rtl_expected.reshape(SPATIAL, OC).astype(np.int16)
    diff = np.abs(expected_s - rtl_s)
    if int(diff.max()) > CREF_TOL:
        raise SystemExit(f"CREF_TOL={CREF_TOL} too small; max diff is {int(diff.max())}")

    body = f"""#ifndef YOLO_CONV3X3_REAL_DATA_H
#define YOLO_CONV3X3_REAL_DATA_H

#include <stdint.h>

#define YOLO_CONV3X3_IN_W {IN_W}u
#define YOLO_CONV3X3_IN_H {IN_H}u
#define YOLO_CONV3X3_IC {IC}u
#define YOLO_CONV3X3_OC {OC}u
#define YOLO_CONV3X3_KH {KH}u
#define YOLO_CONV3X3_KW {KW}u
#define YOLO_CONV3X3_STRIDE {STRIDE}u
#define YOLO_CONV3X3_PAD {PAD}u
#define YOLO_CONV3X3_IN_ZP {IN_ZP}
#define YOLO_CONV3X3_ACT_WORDS {SPATIAL}u
#define YOLO_CONV3X3_WGT_WORDS {OC * KO}u
#define YOLO_CONV3X3_OUT_WORDS {SPATIAL}u
#define YOLO_CONV3X3_REQUANT_MUL {requant_mul}u
#define YOLO_CONV3X3_REQUANT_SHIFT {REQUANT_SHIFT}u
#define YOLO_CONV3X3_REQUANT_ZP {OUT_ZP}
#define YOLO_CONV3X3_CREF_TOL {CREF_TOL}u

{format_u32_array("yolo_conv3x3_act_words", pack_act_words(act))}

{format_u32_array("yolo_conv3x3_wgt_words", pack_wgt_words(weights))}

{format_i32_array("yolo_conv3x3_bias_q", bias_q)}

{format_u32_flat_array("yolo_conv3x3_scale_mul", scale_mul)}

{format_u32_flat_array("yolo_conv3x3_scale_shift", scale_shift)}

static const uint8_t yolo_conv3x3_expected_cref[{SPATIAL}][{OC}] = {{
"""
    for pos in range(SPATIAL):
